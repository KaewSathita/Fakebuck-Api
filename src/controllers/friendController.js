...
exports.deleteFriend = async ( req, res, next ) => {
  try {
    ...
    //SELECT * FROM friends WHERE requester_id = req.user.id AND accepter_id = friendId 
    //OR requester_id = friendId AND accpeter_id = req.user.id
...
exports.updateFriend = async (req, res, next) => {
  try {
    ...
    await Friend.update({ status: FRIEND_ACCEPTED}, 
      {
        where: { 
          status: FRIEND_PENDING, 
